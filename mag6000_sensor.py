...
class Config(Enum):
    IP_ADDRESS_STATION = "10.0.0.92"  # Corregido: sin dos puntos
    MODBUS_PORT = 502
    ...
    MAG_TOTALIZER_ADDRESS = 3014
    MAG_TOTALIZER_ADDRESS_COUNT = 4
    SENSOR_VERSION = "MAG6000"
...
def totaltype_to_realnumber(instance: list, word_order="little") -> float | int:  # Corregido: lowercase
    ...
    try:
        ...
        registers = ModbusTcpClient.convert_to_registers(data_decode, data_type=ModbusTcpClient.DATATYPE.FLOAT32, word_order=word_order)

        data_32 = ModbusTcpClient.convert_from_registers(
            registers=registers,
            data_type=ModbusTcpClient.DATATYPE.FLOAT32,
            word_order=word_order)
        
        logger.info(f"Data decode totalizer 32: {data_32}")

        logger.info(f"Data decode totalizer: {data_decode}")
             
        return round(data_decode,2)
...
def main():
    try:
        with ModbusTcpClient(
            ...
            logger.info(f"📊 Registros Caudal: {Caudal.registers}")
            logger.info(f"📊 Registros Volumen: {Volumen.registers}")
